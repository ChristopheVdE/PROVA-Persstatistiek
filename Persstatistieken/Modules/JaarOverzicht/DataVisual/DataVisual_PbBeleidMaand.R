...
DataVisual.PbBeleidMaand <- function(input, output, session, data, colours, beleid, datadeelbeleid) {
  ...
  # Tabel ------------------------------------------------------------------------ 
    ...
    # Add percentages
    berichten <- data.frame(berichten, "Procentueel" = calc_percentages("beleid.maand", berichten))
    # Return
    return(berichten)
  })
  # Plot ----------------------------------------------------------------------
  berichten.plot <- reactive({
    ...
    for (inhoud in c("Aantal", "Procent")) {
      ...
      if ((inhoud == "Aantal" && input$type.aantal == "Barplot") || (inhoud == "Procent" && input$type.procent == "Barplot")) {
        plots[[inhoud]] <- simple_barplot(Id = 'beleid.maand',
                                          data = df.berichten,
                                          Xaxis = "Maand",
                                          Fill = "Maand",
                                          visual = inhoud,
      ...
      # Taartdiagram ------------------------------------------------------------
        ...
        plots[[inhoud]] <- simple_piechart(Id = 'beleid.maand',
                                           data = df.berichten,
                                           Fill = "Maand",
                                           visual = inhoud,
                                           title = input$title,
