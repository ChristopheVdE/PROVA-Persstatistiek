...
# SERVER =======================================================================
DataVisual.PbVerzenderVerzender <- function(input, output, session, data, colours, verzender) {
...
# Tabel ------------------------------------------------------------------------ 
    ...
    # Add percentages
    berichten <- data.frame(berichten, "Procentueel" = calc_percentages("verzender.alg.verzender", berichten))
    # Return
    return(berichten)
})
  # Plot ----------------------------------------------------------------------
berichten.plot <- reactive({
    ...
    # Barplot -----------------------------------------------------------------
        if ((inhoud == "Aantal" && input$type.aantal == "Barplot") || (inhoud == "Procent" && input$type.procent == "Barplot")) {
          plots[[inhoud]] <- simple_barplot(Id = 'verzender.alg.verzender',
                                       data = df.berichten,
                                       Xaxis = "Verzender",
                                       Fill = "Verzender",
                                       visual = inhoud,
    ...
    # Taartdiagram ------------------------------------------------------------
          ...
          plots[[inhoud]] <- simple_piechart(Id = 'verzender.alg.verzender',
                                             data = df.berichten,
                                             Fill = "Verzender",
                                             visual = inhoud,
                                             title = input$title,
...
ToAppend <- list(
  totaal.plot.aantal = reactive(berichten.plot()$Aantal),
  totaal.plot.procent = reactive(berichten.plot()$Procent),
  totaal.tabel =  reactive(berichten.tabel()), 
  totaal.uitleg = reactive(input$uitleg))
