...
# SERVER =======================================================================
DataVisual.PbConferentieAlgBeleid <- function(input, output, session, data, colours) {
...
# Tabel ------------------------------------------------------------------------ 
    ...
    # Beleid --------------------------------------------------------------
      ...
      # Add percentages
      berichten <- data.frame(berichten, "Procentueel" = calc_percentages("conferentie.alg.beleid", berichten))
      # Return df
...
berichten.plot <- reactive({
    ...
    # Barplot -----------------------------------------------------------------
        if ((inhoud == "Aantal" && input$type.aantal == "Barplot") || (inhoud == "Procent" && input$type.procent == "Barplot")) {
          plots[[inhoud]] <- simple_barplot(Id = 'conferentie.alg.beleid',
                                       data = df.berichten,
                                       Xaxis = "Beleid",
                                       Fill = "Beleid",
                                       visual = inhoud,
    ...
    # Taartdiagram ------------------------------------------------------------
          ...
          plots[[inhoud]] <- simple_piechart(Id = 'conferentie.alg.beleid',
                                             data = df.berichten,
                                             Fill = "Beleid",
                                             visual = inhoud,
                                             title = input$title,
