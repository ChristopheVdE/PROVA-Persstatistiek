...
# SERVER =======================================================================
DataVisual.PbBerichtAlgBeleid <- function(input, output, session, data, colours) {
...
# Tabel ------------------------------------------------------------------------ 
      ...
      # Add percentages
      berichten <- data.frame(berichten, "Procentueel" = calc_percentages("alg.beleid", berichten))
...
berichten.plot <- reactive({
    ...
    # Barplot -----------------------------------------------------------------
        if ((inhoud == "Aantal" && input$type.aantal == "Barplot") || (inhoud == "Procent" && input$type.procent == "Barplot")) {
          plots[[inhoud]] <- simple_barplot(Id = 'alg.beleid',
                                       data = df.berichten,
                                       Xaxis = "Beleid",
                                       Fill = "Beleid",
                                       visual = inhoud,
    ...
    # Taartdiagram ------------------------------------------------------------
          ...
          plots[[inhoud]] <- simple_piechart(Id = 'alg.beleid',
                                             data = df.berichten,
                                             Fill = "Beleid",
                                             visual = inhoud,
                                             title = input$title,
