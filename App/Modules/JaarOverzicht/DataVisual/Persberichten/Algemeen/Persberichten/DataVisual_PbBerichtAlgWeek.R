...
# SERVER =======================================================================
DataVisual.PbBerichtAlgWeek <- function(input, output, session, data, colours) {
...
# Tabel ------------------------------------------------------------------------ 
  ...
  # Add percentages
  berichten <- data.frame(berichten, "Procentueel" = calc_percentages("alg.week", berichten))
...
berichten.plot <- reactive({
    ...
    # Barplot -----------------------------------------------------------------
        if ((inhoud == "Aantal" && input$type.aantal == "Barplot") || (inhoud == "Procent" && input$type.procent == "Barplot")) {
          plots[[inhoud]] <- simple_barplot(Id = 'alg.week',
                                       data = df.berichten,
                                       Xaxis = "Week",
                                       Fill = "Week",
                                       visual = inhoud,
    ...
    # Taartdiagram ------------------------------------------------------------
          ...
          plots[[inhoud]] <- simple_piechart(Id = 'alg.week',
                                             data = df.berichten,
                                             Fill = "Week",
                                             visual = inhoud,
                                             title = input$title,
...
ToAppend <- list(
  week.plot.aantal = reactive(berichten.plot()$Aantal),
  week.plot.procent = reactive(berichten.plot()$Procent),
  week.tabel =  reactive(berichten.tabel()), 
  week.uitleg = reactive(input$uitleg))
